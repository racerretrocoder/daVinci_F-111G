...
var tgt_input = func(n) {
 ...
 #var t_input = ""~ coord_input[0] ~""~ coord_input[1] ~""~ coord_input[2] ~""~ coord_input[3] ~"."~ coord_input[4] ~""~ coord_input[5] ~""~ coord_input[6] ~""~ coord_input[7] ~""~ coord_input[8] ~"";
  ...
  if(n == 1){
    setprop("ai/guided/bomb["~ nr ~"]/target-latitude-deg", ikb_path.getValue());
    weapons.wpn_info[nr].lat = ikb_path.getValue();
    var talt_m = geo.elevation(wpn_info[nr]["lat"], wpn_info[nr]["lon"]);
    if ((talt_m == "nil") or (talt_m == "")) {
      return(weapons.wpn_info[nr].talt_m = -1);
    } else {
        wpn_info[nr].talt_m = talt_m;
      }
    debug.dump(wpn_info[nr]);
  }
  if(n == 2){
    setprop("ai/guided/bomb["~ nr ~"]/target-longitude-deg", ikb_path.getValue());
    weapons.wpn_info[nr].lon = ikb_path.getValue();
    var talt_m = geo.elevation(wpn_info[nr]["lat"], wpn_info[nr]["lon"]);
    if ((talt_m == "nil") or (talt_m == "")) {
      return(weapons.wpn_info[nr].talt_m = -1);
    } else {
